...
# 3. Extract the Cooling Technology column from cooling_data
# Column AL is the 38th column in the cooling_data file.
# Rename it to "Cooling_Technology" for clarity.
cooling_tech <- cooling_data[, 38]
names(cooling_tech) <- "Cooling_Technology"

# 4. Merge the two datasets
# This step assumes there is a common identifier in both datasets (e.g., "Plant_ID").
# Replace 'common_id' below with the actual name of that column.
# If your rows are aligned and there is no common ID, you can bind the column directly.
combined_data <- left_join(power_plants_subset, cooling_tech, by = "common_id")
# If there is no common identifier, use:
# combined_data <- cbind(power_plants_subset, Cooling_Technology = cooling_tech$Cooling_Technology)
...
#    with the power plants data using a common identifier ("common_id"). Update this column name as needed.
#
